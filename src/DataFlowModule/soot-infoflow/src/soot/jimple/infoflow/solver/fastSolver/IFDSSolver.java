...
public class IFDSSolver<N, D extends FastSolverLinkedNode<D, N>, I extends BiDiInterproceduralCFG<N, SootMethod>>
	...
	private void processCall(PathEdge<N, D> edge) {
		...
		if(icfg.isCallStmt(n)){
			SootMethodRef smr = ((Stmt) n).getInvokeExpr().getMethodRef();
			String declaringClz = smr.getDeclaringClass().toString();
//			if(smr.getDeclaringClass().getName().contains("com.anaconda.brave.las")){
//				System.out.println("invokeAll");
//			}

			if(CyfiInfoflowHelper.v().isClassInSupportPackage(declaringClz) || SystemClassHandler.v().isClassInSystemPackage(declaringClz) || CyfiInfoflowHelper.v().isClassInAdditionalIgnore(smr.getDeclaringClass().getName())){
				CyfiInfoflowHelper.v().addNewCalledAPI(smr);
			}
		}
//		if(n.toString().contains("write(byte[])")){
//			System.out.println(n);
//		}


		Collection<SootMethod> callees = CyfiInfoflowHelper.v().getCalleesOf((Unit)n, (IInfoflowCFG) icfg, (Abstraction) d2);
...
//			SootMethod caller = icfg.getMethodOf(target);
//			if(caller.getDeclaringClass().getName().equals("com.example.testcase.MainActivity$8")){
//				System.out.println("inside runnable");
//			}
//			System.out.println(target + " --------- " + icfg.getMethodOf(target));
			if(target == null){
