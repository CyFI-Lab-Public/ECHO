...
public class Infoflow extends AbstractInfoflow {
	...
	private void runAnalysis(final ISourceSinkManager sourcesSinks, final Set<String> additionalSeeds) {
		...
		try {
			...
			CyfiInfoflowHelper.v().patchStatementSinkSourceDefinition(sourcesSinks, this.manager);
			...
			int callGraphSize = Scene.v().getCallGraph().size();
			if(callGraphSize > 10000000) {
				logger.info("Aborting the taint problem for too large CallGraph");
				return;
			}
	...
	private void runTaintAnalysis(final ISourceSinkManager sourcesSinks, final Set<String> additionalSeeds,
		...
		while (hasMoreSources) {
			...
			CyfiInfoflowHelper.v().setInfoflowProblem(forwardProblem);
			...
			try {
				...
				DCLResultHandler.v().filterSourceStmtWithDCLResult(forwardProblem.initialSeeds(), iCfg);
...
//				// DEBUG - remove extra seeds, only keep first one
//				boolean keepFirstSeed = true;
//
//				for (Iterator<Unit> iterator = seeds.keySet().iterator(); iterator.hasNext();) {
//					Unit u = iterator.next();
//
//					if (keepFirstSeed) {
//						SootMethod sm = iCfg.getMethodOf(u);
//						System.out.println("Keeping Soot Method: " + sm.toString());
//						keepFirstSeed = false;  // Set the flag to false after the first iteration
//					} else {
//						System.out.println("Removing seed with Soot Method: " + iCfg.getMethodOf(u).toString());
//						iterator.remove();  // Safely remove the entry using the iterator
//					}
//				}

// At this point, only one seed should remain in the map

				SootClass sc = Scene.v().getSootClassUnsafe("org.apache.http.impl.client.AbstractHttpClient");
				SootClass sc2 = Scene.v().getSootClassUnsafe("com.iapptracker.a");
...
//							final IAbstractionPathBuilder builder = new DebuggingPathBuilder(pathBuilderFactory, manager);
				...
				Chain<SootClass> sms = Scene.v().getClasses();

				HashMap<Integer, SootClass> map = new HashMap<>();
				for(SootClass sm: sms){
					if(SystemClassHandler.v().isClassInSystemPackage(sm) || CyfiInfoflowHelper.v().isClassInSupportPackage(sm.getType())) continue;
					if(sm.getMethods().size() == 0 && sm.getFields().size() == 0) continue;
					int hash = CyfiClassMethodHelper.classHashCode(sm);
					map.put(hash, sm);
				}
	...
	protected boolean isValidSeedMethod(SootMethod sm) {
		if(config.getCyfiConfiguration().getExcludeEntryPointAsSource()){
		...
		if (config.getExcludeSootLibraryClasses() && sm.getDeclaringClass().isLibraryClass())
			return false;
		if(CyfiInfoflowHelper.v().isClassInSupportPackage(sm.getDeclaringClass().getName())){
			return false;
		}
		if(CyfiInfoflowHelper.v().isClassInAdditionalIgnore(sm.getDeclaringClass().getName())){
			return false;
		}
