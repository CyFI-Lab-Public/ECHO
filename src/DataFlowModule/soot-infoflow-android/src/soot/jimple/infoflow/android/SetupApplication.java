...
import soot.jimple.infoflow.cyfi.classmethod.CyfiClassMethodHelper;
import soot.jimple.infoflow.cyfi.entrypoint.CyfiEntryPointHelper;
import soot.jimple.infoflow.cyfi.entrypoint.DCLResultHandler;
import soot.jimple.infoflow.cyfi.infoflow.CyfiInfoflowHelper;
import soot.jimple.infoflow.cyfi.result.DynamicLoadingResultHandler;
import soot.jimple.infoflow.cyfi.result.JSIReportGenerationResultHandler;
import soot.jimple.infoflow.cyfi.sourceNsink.CyfiSourceSinkHelper;
...
public class SetupApplication implements ITaintWrapperDataFlowAnalysis {
	...
	protected void parseAppResources() throws IOException, XmlPullParserException {
		...
		if(!config.getCyfiConfiguration().getUseJSIEntryPoint()){
			...
			this.entrypoints = CyfiEntryPointHelper.v().getJSIEntryPointClasses();
		}
	}
	 ...
	 * @param entryPoint      The entry point for which to calculate the callbacks.
	 *                        Pass null to calculate callbacks for all entry points.
	 * @param entryPointMethod The actual entryPointMethod when the entrypoint is not
	 *                         from a dummy main for the whole class.
	 * @throws IOException            Thrown if the given source/sink file could not
	 *                                be read.
	 * @throws XmlPullParserException Thrown if the Android manifest file could not
	 *                                be read.
	 */
	private void calculateCallbacks(ISourceSinkDefinitionProvider sourcesAndSinks, SootClass entryPoint, SootMethod entryPointMethod)
			throws IOException, XmlPullParserException {
		...
		if (callbackConfig.getEnableCallbacks()) {
			...
			if (callbackFile != null && !callbackFile.isEmpty()) {
				...
				if (cbFile.exists()) {
					...
					if (callbacks != null) {
						...
						createMainMethod(entryPoint, entryPointMethod);
						constructCallgraphInternal();

						createSourceSinkProvider(entryPoint, entryPointMethod, lfp);
						return;
					}
				}
			}
			...
			if (callbackClasses != null && callbackClasses.isEmpty()) {
				...
				case Fast:
					calculateCallbackMethodsFast(lfp, entryPoint, entryPointMethod);
					break;
				case Default:
	 ...
	 */
	protected void createSourceSinkProvider(SootClass entryPoint, SootMethod entryPointMethod, LayoutFileParser lfp) {
		if (this.sourceSinkProvider != null) {
			...
			sourceSinkManager = createSourceSinkManager(lfp, callbacks, entryPointMethod);
		}
	}
	...
	protected ISourceSinkManager createSourceSinkManager(LayoutFileParser lfp,
			Set<AndroidCallbackDefinition> callbacks, SootMethod entryPointMethod) {

		CyfiSourceSinkHelper.addAdditionalSourceNSinkDefToProvider(this.sourceSinkProvider, entryPointMethod, config);
...
//		PackManager.v().getPack("wjpp").apply();
	 ...
	 * 
	 * @param lfp        The layout file parser to be used for analyzing UI controls
	 * @param entryPointClz The entry point for which to calculate the callbacks. Pass
	 *                   null to calculate callbacks for all entry points.
	 * @param entryPointMethod The entry point method for which to calculate the callbacks.
	 * @throws IOException Thrown if a required configuration cannot be read
	 */
	private void calculateCallbackMethodsFast(LayoutFileParser lfp, SootClass entryPointClz, SootMethod entryPointMethod) throws IOException {
		// Construct the current callgraph
		releaseCallgraph();
		createMainMethod(entryPointClz);
		constructCallgraphInternal();

		// Get the classes for which to find callbacks
		Set<SootClass> entryPointClasses = getComponentsToAnalyze(entryPointClz);
	 ...
	 * into the Soot scene.
	 * 
	 * @param entryPointClz The class name of a component to create a main method containing only
	 *            that component, or null to create main method for all components
	 * @param entrypointMethod The method name to build a callgraph with
	 */
	private void createMainMethod(SootClass entryPointClz, SootMethod entrypointMethod) {
		...
		if(entrypointMethod != null){
			Scene.v().setEntryPoints(Collections.singletonList(entrypointMethod));
		} else {
			entryPointCreator = createEntryPointCreator(entryPointClz);
			SootMethod dummyMainMethod = entryPointCreator.createDummyMain();


			List<SootMethod> entryPointMethods = new ArrayList<>();
			entryPointMethods.add(dummyMainMethod);
			if(DCLResultHandler.v().getAdditionalEntrypointMethods().size() > 0){
				entryPointMethods.addAll(DCLResultHandler.v().getAdditionalEntrypointMethods());
			}
...
//			Scene.v().setEntryPoints(Collections.singletonList(dummyMainMethod));
	...
	private void createMainMethod(SootClass entryPointClz) {
		this.createMainMethod(entryPointClz, null);
	}
	...
	public InfoflowResults runInfoflow() throws IOException, XmlPullParserException {
		...
		// flow analysis
		if(config.getCyfiConfiguration().isNoAnalysis()){
			return runInfoflow((ISourceSinkDefinitionProvider) null);
		}
		String dynamicEvaluationResultPath = config.getCyfiConfiguration().getDynamicEvaluationResultPath();

		if(dynamicEvaluationResultPath != null && !dynamicEvaluationResultPath.isEmpty()){
			DCLResultHandler.v().initFromJsonFile(config.getCyfiConfiguration().getDynamicEvaluationResultPath());
		}
	...
	public InfoflowResults runInfoflow(ISourceSinkDefinitionProvider sourcesAndSinks) {
		...
		// read from jsi_of_interest entrypoint list and init jsi classes
		if(config.getCyfiConfiguration().getUseJSIEntryPoint()){
			String jsiInterfaceOfInterestPath = config.getCyfiConfiguration().getJsiInterfaceOfInterestFilePath();
			if(!jsiInterfaceOfInterestPath.equals("")){
				try{
					CyfiEntryPointHelper.v().initJsiInterfaceOfInterestList(jsiInterfaceOfInterestPath);
				}catch (Exception e) {
					logger.error("cannot fetch jsi interface of interest");
					e.printStackTrace();
				}
			}
		}
		...
		if(config.getCyfiConfiguration().isLogClassHashes()){
			try{
				CyfiClassMethodHelper.logClasses(config.getCyfiConfiguration().getLogClassHashesOutputPath());
			}catch(IOException e){
				logger.error("Logging class hashes failed", e);
				throw new RuntimeException("Logging class hashes failed", e);
			}
		}
		if(config.getCyfiConfiguration().isNoAnalysis()){
			return null;
		...
		String extraEntryPointPath = config.getCyfiConfiguration().getExtraEntryPointFilePath();
		if(!extraEntryPointPath.equals("")){
			try{
				CyfiEntryPointHelper.v().initExtraClassList(extraEntryPointPath);
			}catch (Exception e) {
				logger.error("cannot fetch extra entry point");
				e.printStackTrace();
			}
		}
		...
		CyfiInfoflowHelper.v().initBackTraceTargetMap();
		if(!config.getCyfiConfiguration().getUseJSIEntryPoint()){
			entrypoints = DCLResultHandler.v().filterEntryPointClassWithDCLResult(entrypoints);
		}

		// Code for deserialize the dcl results and fit to current entrypoints to be analyzed
		DCLResultHandler.v().initAfterEntrypointMatching();
		...
		if(config.getCyfiConfiguration().getUseJSIEntryPoint()){
			JSIReportGenerationResultHandler.v().setConfig(config);
			this.addResultsAvailableHandler(JSIReportGenerationResultHandler.v());
		}else if(config.getCyfiConfiguration().isDynamicLoading()){
			DynamicLoadingResultHandler.v().outputFilePath = config.getAnalysisFileConfig().getOutputFile();
			this.addResultsAvailableHandler(DynamicLoadingResultHandler.v());
		}
		...
		if (config.getOneComponentAtATime()) {
			if(config.getCyfiConfiguration().getUseJSIEntryPoint()){
				List<SootMethod> entryPointMethodList = new ArrayList<>();
				for(SootClass sc: entrypoints){
					entryPointMethodList.addAll(CyfiEntryPointHelper.v().getJSIEntrypoints(sc));
				}
				while(!entryPointMethodList.isEmpty()){
					SootMethod entryPointMethod = entryPointMethodList.remove(0);
					SootClass entryPointClz = entryPointMethod.getDeclaringClass();
					CyfiInfoflowHelper.v().setEntryPointMethod(entryPointMethod);
					processEntryPoint(sourcesAndSinks, resultAggregator, entryPointMethodList.size(), entryPointClz, entryPointMethod);
				}
			}else {
				...
				while (!entrypointWorklist.isEmpty()) {
					...
					CyfiInfoflowHelper.v().setEntryPointClass(entrypoint);
					try {
						processEntryPoint(sourcesAndSinks, resultAggregator, entrypointWorklist.size(), entrypoint, null);
					} catch(Exception e){
						logger.error("Error during EntryPoint: " + entrypoint.getName() ,e);
					}
				}
				int extraCount = -1;
				for(SootMethod sm: CyfiEntryPointHelper.v().getExtraEntryPointMethods()){
					CyfiInfoflowHelper.v().setEntryPointMethod(sm);
					processEntryPoint(sourcesAndSinks, resultAggregator, extraCount, sm.getDeclaringClass(), sm);
					extraCount -= 1;
				}
	...
	protected void processEntryPoint(ISourceSinkDefinitionProvider sourcesAndSinks,
			MultiRunResultAggregator resultAggregator, int numEntryPoints, SootClass entrypoint, SootMethod entryPointMethod) {
		...
		try {
			if (config.getOneComponentAtATime())
				calculateCallbacks(sourcesAndSinks, entrypoint, entryPointMethod);
			else
...
//			if(src.getDeclaringClass().getName().equals("b.a.a.l")){
//			}
//		}
//		Collection<SootClass> scs = Scene.v().getClasses().stream().filter(sc -> !CyfiInfoflowHelper.v().isClassInAdditionalIgnore(sc.getName())).collect(Collectors.toList());
//		System.out.println("Classes: " + scs.size());
//
//		System.exit(1);
	...
	protected void serializeResults(InfoflowResults results, IInfoflowCFG cfg) {
		...
		if (resultsFile != null && !resultsFile.isEmpty()) {
			String outputFormat = config.getCyfiConfiguration().getOutputFormat().toLowerCase();
			switch(outputFormat) {
				case "xml":
					if (config.getCyfiConfiguration().getOutputFormat().equals("xml")) {
				...
				case "json":
					if(config.getCyfiConfiguration().getUseJSIEntryPoint()){
						JSIReportGenerationResultHandler.v().saveResultToFile(resultsFile);
					} else if(config.getCyfiConfiguration().isDynamicLoading()) {
						DynamicLoadingResultHandler.v().saveResultToFile(resultsFile);
					}
					break;
			}
		}
...
//		if(config.getCyfiConfiguration().getUseJSIEntryPoint()) {
//			JSIReportGenerationResultHandler.v().outputFilePath = config.getAnalysisFileConfig().getOutputFile();
//			info.addResultsAvailableHandler(JSIReportGenerationResultHandler.v());
//		}
