...
public class MainClass {
	...
	// cyfi added options
	private static final String OPTION_JSI_ENTRYPOINT = "js";
	private static final String OPTION_OUTPUT_FORMAT = "of";
	private static final String OPTION_LOG_CALL_METHOD_ALONG_RESULTS = "logcall";
	private static final String OPTION_DYNAMIC_LOADING = "dl";
	private static final String OPTION_FULL_ENTRYPOINT_LIST = "fullep";
	private static final String OPTION_NO_ANALYSIS = "na";
	private static final String OPTION_LOG_ALL_CLASS_HASH = "logclz";
	private static final String OPTION_LOG_CLASS_PATH = "logclzpath";
	private static final String OPTION_EXTRA_ENTRYPOINT_LIST_FILE_PATH = "extraentrypoint";

	private static final String OPTION_DYNAMIC_EVALUATION_RESULT_FILE_PATH = "dynamicevaluationresult";

	private static final String OPTION_JSI_INTERFACE_OF_INTEREST_LIST_FILE_PATH = "jsiinterfaceofinterest";
	protected MainClass() {
	...
	private void initializeCommandLineOptions() {
		...
		options.addOption(OPTION_OUTPUT_FILE, "outputfile", true, "Output file for the discovered data flows");
		options.addOption(OPTION_ADDITIONAL_CLASSPATH, "additionalclasspath", true,
				"Additional JAR file that shall be put on the classpath");
		...
		// cyfi added options
		options.addOption(OPTION_JSI_ENTRYPOINT, "javascriptInterface", false,
				"Use js interface methods as entrypoints");
		options.addOption(OPTION_OUTPUT_FORMAT, "outputformat", true, "Indicate the output format as json or xml");
		options.addOption(OPTION_LOG_CALL_METHOD_ALONG_RESULTS, "logcallmethod", false, "Whether log all called system api along taint analysis");
		options.addOption(OPTION_DYNAMIC_LOADING, "dynamicloading", false, "running in Dynamic loading mode");
		options.addOption(OPTION_FULL_ENTRYPOINT_LIST, "fullentrypoint", false, "consider all component as entry points");
		options.addOption(OPTION_NO_ANALYSIS, "noanalysis", false, "Do not run analysis");
		options.addOption(OPTION_LOG_ALL_CLASS_HASH,  "logallclasshash", false, "log all classes hashes");
		options.addOption(OPTION_LOG_CLASS_PATH, "logclasspath", true, "log class path");
		options.addOption(OPTION_EXTRA_ENTRYPOINT_LIST_FILE_PATH, "extraentrypoint", true, "extra entry point list");
		options.addOption(OPTION_DYNAMIC_EVALUATION_RESULT_FILE_PATH, "dynamicevaluationresult", true, "dynamic evaluation result file path");
		options.addOption(OPTION_JSI_INTERFACE_OF_INTEREST_LIST_FILE_PATH, "jsiinterfaceofinterest", true, "jsi interface of interest from dynamic call logging");
	}
	...
	private void parseCommandLineOptions(CommandLine cmd, InfoflowAndroidConfiguration config) {
		...
		//cyfi options
		if(cmd.hasOption(OPTION_JSI_ENTRYPOINT)) {
			config.getCyfiConfiguration().setUseJSIEntryPoint(true);
		}

		if(cmd.hasOption(OPTION_OUTPUT_FORMAT)){
			config.getCyfiConfiguration().setOutputFormat(cmd.getOptionValue(OPTION_OUTPUT_FORMAT));
		}

		if(cmd.hasOption(OPTION_LOG_CALL_METHOD_ALONG_RESULTS)){
			config.getCyfiConfiguration().setLogCallMethod(true);
		}

		if(cmd.hasOption(OPTION_DYNAMIC_LOADING)) {
			config.getCyfiConfiguration().setDynamicLoading(true);
		}

		if(cmd.hasOption(OPTION_FULL_ENTRYPOINT_LIST)){
			config.getCyfiConfiguration().setUseFullEntryPoint(true);
		}

		if(cmd.hasOption(OPTION_NO_ANALYSIS)){
			config.getCyfiConfiguration().setNoAnalysis(true);
		}

		if(cmd.hasOption(OPTION_LOG_ALL_CLASS_HASH)){
			config.getCyfiConfiguration().setLogClassHashes(true);
		}

		if(cmd.hasOption(OPTION_LOG_CLASS_PATH)){
			config.getCyfiConfiguration().setLogClassHashesOutputPath(cmd.getOptionValue(OPTION_LOG_CLASS_PATH));
		}

		if(cmd.hasOption(OPTION_EXTRA_ENTRYPOINT_LIST_FILE_PATH)){
			config.getCyfiConfiguration().setExtraEntryPointFilePath(cmd.getOptionValue(OPTION_EXTRA_ENTRYPOINT_LIST_FILE_PATH));
		}
		...
		// get external dynamic evaluation results
		if(cmd.hasOption(OPTION_DYNAMIC_EVALUATION_RESULT_FILE_PATH)){
			String path = cmd.getOptionValue(OPTION_DYNAMIC_EVALUATION_RESULT_FILE_PATH);
			if(new File(path).exists()){
				config.getCyfiConfiguration().setDynamicEvaluationResultPath(path);
			}
		}

		// get list of jsi interfaces of interest
		if(cmd.hasOption(OPTION_JSI_INTERFACE_OF_INTEREST_LIST_FILE_PATH)){
			String path = cmd.getOptionValue(OPTION_JSI_INTERFACE_OF_INTEREST_LIST_FILE_PATH);
			if(new File(path).exists()){
				config.getCyfiConfiguration().setJsiInterfaceOfInterestFilePath(path);
			}
		}
	}
